...
local SIKH_LOAD_SCREEN_UNIT_ICONS:table = {
	UNIT_SIKH_AKALI_NIHANG = "ICON_UNIT_SIKH_AKALI_NIHANG_PORTRAIT",
	UNIT_SIKH_MISLDAR_CAVALRY = "ICON_UNIT_SIKH_MISLDAR_CAVALRY_PORTRAIT",
};
...
function OnLoadScreenContentReady()
	...
	if playerConfig == nil then
		...
		for _, item in ipairs(uniqueUnits) do
			...
			iconAtlas = SIKH_LOAD_SCREEN_UNIT_ICONS[item.Type] or "ICON_"..item.Type;
...
function OnLoadGameViewStateDone()
	...
	if m_isResyncLoad or GameConfiguration.IsAnyMultiplayer() or GameConfiguration:IsWorldBuilderEditor() then
		...
		-- CVI-1048 - These callbacks are not cleaned up correctly in multiplayer Mac games and cause a crash.
		-- Only registering them on single player games fixes that bug.
